...
USAGE: python setup_prerequisites.py <COMPONENT_NAME>

PARAMETERS:
  COMPONENT_NAME - Name for the component (e.g., "canny", "hubspot-crm")
...
def prompt_install_prism():
    ...
    print("")
    print("Prism CLI is required to build and publish components.")
    print("")
...
def print_final_status(
    success, component_name, user_info=None, project_dir=None, session_dir=None
):
    ...
    if success and user_info and project_dir:
        component_dir = f"{project_dir}/components/{component_name}"
...
def main():
    print("Prismatic Component Builder - Phase 1 Setup")
    print("")
    ...
    # Check for component name argument
    if len(sys.argv) < 2:
        print("")
        print("=" * 60)
        ...
        print("=" * 60)
        print("")
        print("Usage: python setup_prerequisites.py <COMPONENT_NAME>")
        print("")
        print("Parameters:")
        print(
            "  COMPONENT_NAME - Name for the component (e.g., 'canny', 'hubspot-crm')"
        ...
        print("")
        print("Examples:")
        print("  python setup_prerequisites.py canny")
        print("  python setup_prerequisites.py my-utility")
        return 1
    ...
    if not version:
        ...
        if prompt_install_prism():
            ...
            if not install_prism():
                print_final_status(False, component_name)
                return 1
        else:
            print_final_status(False, component_name)
            return 1
    ...
    session_dir = ensure_session_directory(project_dir, component_name)

    # Success!
    print_final_status(True, component_name, user_info, project_dir, session_dir)
    return 0
