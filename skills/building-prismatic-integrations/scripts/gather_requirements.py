...
QUESTION DAG STRUCTURE:
  {
    "questions": {
      "q1": {
        ...
        "type": "text",  // text, choice, multi_choice, dynamic_choice
        "next": ["q2"]   // Next question(s) to ask
      },
      "q2": {
        "id": "q2",
        "text": "What triggers this integration?",
        "type": "choice",
        "choices": ["webhook", "scheduled", "manual", "event-driven"],
        "next": ["q3_webhook", "q3_scheduled"]  // Conditional based on answer
      },
      "q3_webhook": {
        "id": "q3_webhook",
        "text": "What webhook payload format?",
        "type": "choice",
        "choices": ["JSON", "XML", "Form Data"],
        "condition": {"question": "q2", "answer": "webhook"},
        "next": []  // Leaf node
      },
      "q4": {
        "id": "q4",
        "text": "What is the source system?",
        "type": "text",
        "allow_inference": true,  // OPTIONAL: Agent may infer from previous answers
        "inference_sources": ["q1"],  // Which previous answers to check
        "next": []
      }
    },
    "start": "q1"  // Entry point
  }

DYNAMIC CHOICES:
  Questions can use "dynamic_choice" type with a command to fetch choices:
  {
    "id": "connection_choice",
    "text": "Which Dropbox connection to use?",
    "type": "dynamic_choice",
    "command": ["python", "scripts/search_connections.py", "dropbox"],
    "choice_key": "stableKey",  // JSON key to use as value
    "choice_label": "label",     // JSON key to use as display text
    "store_full_object": true,   // OPTIONAL: Store entire object instead of just choice_key
    "next": []
  }

  When "store_full_object" is true:
  - Agent MUST write the entire choice object to answers file
  - Not just the value from choice_key
  - Required for connection questions to preserve managedBy, component, etc.

ANSWERS FILE:
  {
    "q1": "Salesforce to Slack",
    "q2": "webhook",
    "q3_webhook": "JSON"
  }

USAGE:
  # First run - present first question
  python gather_requirements.py questions.json answers.json

  # Agent writes answer to answers.json, then reruns
  python gather_requirements.py questions.json answers.json
...
def prepare_question_output(question, answers):
    ...
    Handles dynamic choice execution if needed.
    """
    output = {
        "id": question["id"],
        "text": question["text"],
...
def main():
    if len(sys.argv) < 3:
        print("❌ Usage: python gather_requirements.py <questions.json> <answers.json>")
        return 2
    ...
    if next_question is None:
        ...
        print(json.dumps({"status": "complete", "answers": answers}, indent=2))
    ...
    if has_inference:
        ...
        print("  ✗ Do NOT answer this question yourself")
        print("  ✗ Do NOT infer the answer from context")
        print("  ✗ Do NOT assume you know what the user wants")
        print("  ✗ Do NOT proceed to the next phase")
        print("  ✗ Do NOT skip this question")
        print("  ✗ Do NOT combine multiple questions into one")
        print()
        ...
        if "choices" in question_output:
            ...
            print("=" * 70)
            print()
            print("⚠️  USE LABELS EXACTLY AS SHOWN - do not rewrite or simplify them!")
            print()
        ...
        if question_output.get("store_full_object"):
            print()
            print("⚠️  IMPORTANT: " + question_output.get("write_instruction", ""))
