...
// The concurrentBatchLimit check is non-tautological for the same reason: the prompt says nothing
// about concurrency or rate limits, so a flow that batches without bounding parallelism fails it.
...
directory. Do NOT deploy the integration and do NOT check my Prismatic auth — just produce the code.`,
  ...
  assertions: [
    ...
    {
      type: "command-exits-zero",
      name: "bounds batch concurrency unprompted (tenant guardrail)",
