...
per-batch executions, add `batchConfig: { batchSize, concurrentBatchLimit }` and build the flow's
`trigger` with `batchFlowTrigger` (import it alongside `flow`). Each fire returns
`{ items, paginationState? }`. Always set `concurrentBatchLimit` — without it a single large fire
dispatches unlimited batches and can consume the tenant's execution slots. See
`references/cni-examples/batch-flows.md`.
...
// Run: node scripts/integrations/scaffold-project.ts <name> --components slack
